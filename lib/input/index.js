...
export default class Input {
  ...
  constructor(element, opts) {
    ...
    this._containerStyle = document.getElementById('cocos3dContainer').style;
    ...
    this._touchstartHandle = event => {
      ...
      for (let i = 0; i < event.changedTouches.length; i++) {
        ...
        touch.x = this._calcWebOffsetX(changedTouch.clientX, changedTouch.clientY);
        touch.y = this._calcWebOffsetY(changedTouch.clientX, changedTouch.clientY);
        touch.dx = 0;
        touch.dy = 0;
        touch.prevX = 0;
        touch.prevY = 0;
      ...
      for (let i = 0; i < this._touches.length; i++) {
        for (let j = 0; j < event.changedTouches.length; j++) {
          ...
          if (touch.id === changedTouch.identifier) {
            ...
            touch.prevX = touch.x = this._calcWebOffsetX(changedTouch.clientX, changedTouch.clientY);
            touch.prevY = touch.y = this._calcWebOffsetY(changedTouch.clientX, changedTouch.clientY);
            touch.dx = 0;
            touch.dy = 0;
          }
        }
  ...
  */
  ...
  _calcWebOffsetX(clientX, clientY) {
    let rotate = this._containerStyle.transform;
  ...
  _calcWebOffsetY(clientX, clientY) {
    let rotate = this._containerStyle.transform;
    let deg = rotate && rotate.match(/[\-|\d]\d{1,}/);
    if (deg === null || deg === undefined) {
      deg = '0';
